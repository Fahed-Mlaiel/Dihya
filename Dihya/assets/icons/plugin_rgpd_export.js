// Plugin d’export RGPD des métadonnées des icônes Dihya (Node.js)
const fs = require('fs');
const path = require('path');
const meta = require('./meta_icons.json');

function exportRGPD(format = 'json') {
  const out = path.join(__dirname, `meta_icons_export.${format}`);
  if (format === 'json') {
    ...
    // Simple YAML export (sans dépendance)
    let yaml = '';
    for (const k in meta) {
