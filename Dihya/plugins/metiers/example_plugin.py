"""
Exemple de plugin métier Dihya (Python)
...
class ExamplePlugin:
    def __init__(self, roles=None, i18n=None):
        ...
        self.i18n = i18n or {
            'fr': 'Plugin métier exemple',
            'en': 'Sample business plugin',
            'ar': 'إضافة مهنية نموذجية',
            'tzr': 'Plugin amasal amasal'
        }

    def init(self, context):
        context.log('Initialisation du plugin métier Dihya', self.i18n)
    ...
    def on_load(self, context):
        context.log('Plugin métier chargé', self.i18n)
        # Audit, extension, fallback souverain

    def on_save(self, context, data):
        context.log('Données métier sauvegardées', {**self.i18n, 'data': data})
        # Sécurité, RGPD, extension

    def on_export(self, context, format):
        context.log('Export métier', {'format': format, **self.i18n})
        # Accessibilité, multilingue, fallback

    def on_audit(self, context, event):
        context.log('Audit métier', {'event': event, **self.i18n})
        # Journalisation souveraine, conformité

    def on_error(self, context, error):
        context.log('Erreur plugin métier', {'error': str(error), **self.i18n})
...
# Voir tests/plugins/metiers/test_example_plugin.py pour la couverture complète (hooks, sécurité, i18n, extension, fallback)
