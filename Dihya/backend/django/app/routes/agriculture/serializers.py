...
class CapteurSerializer(serializers.Serializer):
    id = serializers.IntegerField(read_only=True, help_text=_('Identifiant unique / Unique ID / المعرف الفريد / ⴰⵎⴰⵣⵉⵖ'), example=1)
    type = serializers.CharField(max_length=255, help_text=_('Type de capteur (ex : température, humidité, pH) / Sensor type / نوع المستشعر / ⴰⵙⵉⵏⴰⴹ'), example='température')
    valeur = serializers.FloatField(help_text=_('Valeur mesurée / Measured value / القيمة المقاسة / ⴰⵙⵉⵏⴰⴹ n tazmert'), example=23.5)
    unite = serializers.CharField(max_length=50, help_text=_('Unité de mesure (ex : °C, %, pH) / Unit / وحدة القياس / ⴰⵙⵉⵏⴰⴹ n tazmert'), example='°C')
    date_mesure = serializers.DateTimeField(help_text=_('Date de mesure (UTC) / Measurement date / تاريخ القياس / ⴰⵙⴳⴰⵙ n tazmert'), example='2025-05-22T10:00:00Z')
    exploitation = serializers.IntegerField(help_text=_('ID exploitation / Farm ID / معرف الاستغلال / ⴰⵎⴰⵣⵉⵖ n timgart'), example=42)
