"""
test.py - Tests unitaires et d'intégration pour la gestion avancée des quotas IA (Python)
Sécurité maximale, multitenancy, i18n, audit, mock IA fallback, couverture complète
"""
import pytest
from flask import Flask
from app.ai_fallback.quotas.routes import quotas_bp
from app.utils.test_helpers import get_jwt, setup_test_user, teardown_test_user
...
def test_fallback_on_quota_exceeded(client, admin_token, monkeypatch):
    ...
    monkeypatch.setattr('app.ai_fallback.quotas.routes.consume_quota', mock_consume)
