...
Bonnes pratiques :
- Logger tout accès critique, modification de permissions, échec d’authentification, etc.
- Ne jamais logguer de secrets ou de données personnelles.
- Protéger l’accès à la consultation des logs (ACL, IP whitelist, etc.).
- Prévoir des tests unitaires pour chaque fonction critique.
"""
...
from app.security.acl import require_permission
...
def log_security_event(event_type: str, user: str = "unknown", details: str = ""):
    """
    Journalise un événement de sécurité dans le fichier d’audit.

    Args:
        event_type (str): Type d’événement (ex: 'login_failed', 'permission_change').
        user (str): Identifiant utilisateur (anonymisé si besoin).
        details (str): Détails additionnels (jamais de secret !).
    """
