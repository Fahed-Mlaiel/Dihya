...
def create_project():
    ...
    if not result:
        result = ia_fallback('energie', data)
    return jsonify(result), 201

# GraphQL endpoint (exemple)
from flask_graphql import GraphQLView
from ...graphql.energie_schema import schema as energie_schema
energie_bp.add_url_rule('/graphql', view_func=GraphQLView.as_view('graphql', schema=energie_schema, graphiql=True))
