"""
Mobile Project Management Routes
RESTful & GraphQL API for mobile, hybrid apps, PWA project management.
...
POST /mobile/projects/ia/generate
POST /mobile/graphql
...
@mobile_bp.route('/mobile/projects', methods=['GET'])
@jwt_required()
@roles_required(['admin', 'user'])
@multitenant
@validate_request
@plugin_hook('mobile_list')
def list_projects():
    """List all mobile projects"""
    # ... fetch projects from DB ...
    return jsonify({'projects': [], 'msg': translate('projects_listed')})

@mobile_bp.route('/mobile/projects', methods=['POST'])
@jwt_required()
@roles_required(['admin'])
@multitenant
@validate_request
@plugin_hook('mobile_create')
def create_project():
    """Create a new mobile project"""
    # ... create project logic ...
    return jsonify({'msg': translate('project_created')}), 201

@mobile_bp.route('/mobile/projects/<int:pid>', methods=['GET'])
@jwt_required()
@roles_required(['admin', 'user', 'invite'])
@multitenant
@validate_request
@plugin_hook('mobile_get')
def get_project(pid: int):
    """Get project details"""
    # ... get project logic ...
    return jsonify({'project': {}, 'msg': translate('project_fetched')})

@mobile_bp.route('/mobile/projects/<int:pid>', methods=['PUT'])
@jwt_required()
@roles_required(['admin'])
@multitenant
@validate_request
@plugin_hook('mobile_update')
def update_project(pid: int):
    """Update project details"""
    # ... update project logic ...
    return jsonify({'msg': translate('project_updated')})

@mobile_bp.route('/mobile/projects/<int:pid>', methods=['DELETE'])
@jwt_required()
@roles_required(['admin'])
@multitenant
@validate_request
@plugin_hook('mobile_delete')
def delete_project(pid: int):
    """Delete a project (anonymized if RGPD)"""
    # ... delete/anonymize logic ...
    return jsonify({'msg': translate('project_deleted')})

@mobile_bp.route('/mobile/projects/ia/generate', methods=['POST'])
@jwt_required()
@roles_required(['admin', 'user'])
@multitenant
@validate_request
@plugin_hook('mobile_ia_generate')
...
@mobile_bp.route('/mobile/projects/export', methods=['GET'])
@jwt_required()
@roles_required(['admin'])
@multitenant
@plugin_hook('mobile_export')
def export_projects():
    """Export all projects (RGPD compliant)"""
    data = export_data('mobile')
    return jsonify({'data': data, 'msg': translate('export_ok')})

@mobile_bp.route('/mobile/graphql', methods=['POST'])
@jwt_required()
@roles_required(['admin', 'user'])
@multitenant
@plugin_hook('mobile_graphql')
def mobile_graphql():
    """GraphQL endpoint for mobile projects"""
    query = request.json.get('query')
    variables = request.json.get('variables')
    result = graphql_query('mobile', query, variables)
