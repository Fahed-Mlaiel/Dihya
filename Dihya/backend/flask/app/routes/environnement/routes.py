"""
Routes Environnement - API RESTful & GraphQL
Gestion avancée de projets environnement (biodiversité, pollution, IA).
...
@environnement_bp.route('/sites', methods=['GET'])
@jwt_required()
@role_required(['admin', 'user'])
@audit_log
def list_sites():
    """Liste des sites environnementaux (multitenant, filtré, paginé)."""
    # ... récupération sites ...
    return jsonify({'sites': [], 'msg': _('Liste des sites')})

@environnement_bp.route('/sites', methods=['POST'])
@jwt_required()
@role_required(['admin'])
...
@environnement_bp.route('/sites/<int:sid>', methods=['GET'])
@jwt_required()
@role_required(['admin', 'user', 'invite'])
@audit_log
def get_site(sid: int):
    """Détail d'un site environnemental."""
    # ... récupération site ...
    return jsonify({'site': {}, 'msg': _('Détail site')})

@environnement_bp.route('/sites/<int:sid>', methods=['PUT'])
@jwt_required()
@role_required(['admin'])
@validate_json(['name', 'type', 'location'])
@audit_log
def update_site(sid: int):
    """Mise à jour d'un site environnemental."""
    # ... mise à jour ...
    return jsonify({'msg': _('Site mis à jour')})

@environnement_bp.route('/sites/<int:sid>', methods=['DELETE'])
@jwt_required()
@role_required(['admin'])
@audit_log
...
# GraphQL endpoint
environnement_bp.add_url_rule('/graphql', view_func=graphql_view('environnement'))

# Plugin extensibility
@environnement_bp.route('/plugins', methods=['POST'])
@jwt_required()
@role_required(['admin'])
@audit_log
def add_plugin():
    """Ajout dynamique de plugin environnement."""
    plugin_hook(request.json)
    return jsonify({'msg': _('Plugin ajouté')})

# IA integration (fallback)
@environnement_bp.route('/ia/generate', methods=['POST'])
@jwt_required()
@role_required(['admin', 'user'])
@audit_log
def ia_generate():
...
# RGPD export
@environnement_bp.route('/export', methods=['GET'])
@jwt_required()
@role_required(['admin'])
@audit_log
def export_sites():
...
@environnement_bp.route('/robots.txt', methods=['GET'])
def robots():
    return current_app.send_static_file('robots.txt')

@environnement_bp.route('/sitemap.xml', methods=['GET'])
def sitemap():
