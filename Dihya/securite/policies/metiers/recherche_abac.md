...
## Description
Politique d’accès basée sur les attributs (ABAC) pour la gestion recherche dans Dihya. Contrôle fin selon le rôle, le contexte, la conformité RGPD, et la localisation.
...
- `resource` : projet, publication, donnée
- `locale` : fr, en, ar, ...
...
```python
"""
Vérification d’accès recherche (ABAC)
:param user: dict (doit contenir role, tenant, gdpr_consent)
:param action: str
:param resource: str
:return: bool
"""
def check_recherche_access(user: dict, action: str, resource: str) -> bool:
    # ...existing code...
